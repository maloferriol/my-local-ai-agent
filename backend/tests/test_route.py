...
def test_ensure_tool_registry_initialized():
    """Test tool registry initialization."""
    with patch("src.agent.my_local_agent.route.tool_registry", None):
        with patch("src.agent.my_local_agent.route.create_configured_agent_registry") as mock_create:
            mock_registry = MagicMock()
            mock_registry.tools = {"test_tool": MagicMock()}
...
def test_get_tool_stats_endpoint(test_client):
    ...
    with patch("src.agent.my_local_agent.route.tool_registry") as mock_registry:
        mock_registry.get_tool_stats.return_value = {"total_tools": 5, "active_tools": 3}
...
def test_get_enhanced_conversation_summary_not_found(test_client):
    ...
    with patch("src.agent.my_local_agent.route.ConversationManager.load_existing") as mock_load:
...
def test_stream_model_response_thinking_content():
    ...
    async def run_test():
        with patch("src.agent.my_local_agent.route.tracer.start_as_current_span") as mock_span:
            ...
            with patch("src.agent.my_local_agent.route.ollama_client") as mock_client:
                async def mock_chat_generator():
                    yield {"message": {"thinking": "thinking...", "content": "part1"}, "done": False}
...
def test_execute_tools_successful_execution():
    ...
    async def run_test():
        mock_conv_manager = MagicMock()
        tool_calls = [{"function": {"name": "working_tool", "arguments": {"param": "value"}}}]
        ...
        with patch("src.agent.my_local_agent.route.tracer.start_as_current_span") as mock_span:
            ...
            with patch("src.agent.my_local_agent.route.tool_registry") as mock_registry:
                ...
                mock_tool.average_execution_time_ms = 100
                mock_registry.get_tool.return_value = mock_tool
                mock_registry.execute_tool.return_value = "tool result"
...
def test_stream_chat_with_tools_model_error():
    ...
    async def run_test():
        ...
        with patch("src.agent.my_local_agent.route._stream_model_response") as mock_stream:
            mock_stream.side_effect = Exception("Model streaming error")

            parent_ctx = MagicMock()
            generator = _stream_chat_with_tools_refactored("test-model", mock_conv_manager, parent_ctx)
...
def test_invoke_streaming_response_error(mock_stream, test_client, mock_conversation_manager):
...
def test_invoke_with_thinking_model(test_client, mock_conversation_manager):
    ...
    with patch("src.agent.my_local_agent.route._stream_chat_with_tools_refactored") as mock_stream:
...
def test_app_lifespan_startup_and_shutdown():
    ...
    with patch("src.agent.my_local_agent.route.DatabaseManager") as mock_db:
        ...
        with patch("src.agent.my_local_agent.route.create_configured_agent_registry") as mock_create:
            mock_registry = MagicMock()
            mock_registry.tools = {"tool1": MagicMock(), "tool2": MagicMock()}
            mock_create.return_value = mock_registry

            # Test that we can create a test client (triggers lifespan)
            with TestClient(app):
                # Verify startup was called during lifespan
                mock_create.assert_called_once()