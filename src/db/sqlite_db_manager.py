...
# logging.basicConfig(filename='logs/sqlite_db_manager.log', encoding='utf-8', level=logging.DEBUG)

# Get your logger instance for this module
logger = logging.getLogger('db_sqlite_logger')

logger.info("DB Sqlite logger initialized and logging is configured.")
...
class DatabaseManager:
    ...
    def insert_message(self, conversation_id: int, step: int, role: str, content: str, thinking: str = '', tool_calls: str = '', tool_results: str = '', model: str = '', tool_name: str = ''):
        """Inserts a message into the messages table."""
        try:
            self.execute_query('''
                INSERT INTO messages (conversation_id, step, role, content, thinking, tool_name, tool_calls, tool_results, model)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (conversation_id, step, role, content, thinking, tool_name, tool_calls, tool_results, model))
            logger.info('Inserted message for conversation_id %d at step %d', conversation_id, step)
        except sqlite3.Error as e:
    ...
    def get_messages(self, conversation_id: int, limit: int = 10):
        ...
        try:
            return self.fetch_all('SELECT * FROM messages WHERE conversation_id = ? ORDER BY id DESC LIMIT ?', (conversation_id, limit))
        except sqlite3.Error as e:
            logger.error('Error fetching messages for conversation_id %d: %s', conversation_id, e)
            return []
    ...
    def get_conversations(self, limit: int = 10, offset: int = 0):
        ...
        try:
            return self.fetch_all('SELECT * FROM conversations ORDER BY id DESC LIMIT ? OFFSET ?', (limit, offset))
        except sqlite3.Error as e:
    ...
    def create_conversation(self, title: str  = '',) -> int:
        ...
        try:
            random_title = title if title != '' else DatabaseUtils.generate_random_name(3)
            conversation_id = self.execute_query('INSERT INTO conversations (title) VALUES (?)', (random_title,))
