...
class ConversationManager:
    ...
    def start_new_conversation(self, model: str = None, title: str = None) -> int:
        ...
        conversation_logger.info("Started new conversation with ID: %s", conversation_id)
    ...
    def add_user_message(self, content: str, model: str = None) -> Message:
        ...
        conversation_logger.debug("Added user message: %s", content[:50] + "..." if len(content) > 50 else content)
        return message
    
    def add_assistant_message(self, content: str, thinking: str = None, model: str = None) -> Message:
        """
        ...
        conversation_logger.debug("Added assistant message: %s", content[:50] + "..." if len(content) > 50 else content)
        return message
    
    def add_tool_message(self, content: str, tool_name: str, model: str = None) -> Message:
        """
        ...
        conversation_logger.debug("Added tool message from %s: %s", tool_name, content[:50] + "..." if len(content) > 50 else content)
        return message
    ...
    def load_conversation(self, conversation_id: int) -> Optional[Conversation]:
        ...
        conversation_logger.info("Loaded conversation %s with %d messages", conversation_id, len(conversation.messages))
        return conversation
    ...
    def update_conversation_title(self, title: str):
        ...
        if hasattr(self.db_manager, 'update_conversation_title'):
            self.db_manager.update_conversation_title(self.current_conversation.id, title)
    ...
    def get_conversation_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'last_message': self.current_conversation.get_last_message().content if self.current_conversation.get_last_message() else None
        }
    ...
    def close_conversation(self):
        ...
        if self.current_conversation:
            conversation_logger.info("Closing conversation %s", self.current_conversation.id)
