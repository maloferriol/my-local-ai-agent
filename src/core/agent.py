"""
Core AI agent logic.
Manages the agent's behavior, tool execution, and coordinates with the conversation manager.
"""
...
class Agent:
    ...
    def _process_user_input(self, user_input: str, conversation_messages: list):
        ...
        response_content, thinking = self._generate_ai_response(conversation_messages, step, conversation_id)
        ...
        self._append_assistant_message_with_thinking(conversation_messages, response_content, thinking)
        
        logger.debug('Conversation length: %d', len(conversation_messages))
    
    def _generate_ai_response(self, conversation_messages: list, step: int, conversation_id: int) -> tuple[str, str]:
        """
        ...
        if self.stream:
            return self._generate_streaming_response(conversation_messages, step, conversation_id)
        else:
            return self._generate_non_streaming_response(conversation_messages, step, conversation_id)
    
    def _generate_streaming_response(self, conversation_messages: list, step: int, conversation_id: int) -> tuple[str, str]:
        ...
        if hasattr(chunk.message, 'tool_calls') and chunk.message.tool_calls:
            self._handle_tool_calls(chunk.message.tool_calls, conversation_messages, step, conversation_id)
        
        return full_response, thinking
    
    def _generate_non_streaming_response(self, conversation_messages: list, step: int, conversation_id: int) -> tuple[str, str]:
        ...
        if hasattr(response.message, 'tool_calls') and response.message.tool_calls:
            self._handle_tool_calls(response.message.tool_calls, conversation_messages, step, conversation_id)
        
        return full_response, thinking
    
    def _handle_tool_calls(self, tool_calls, conversation_messages: list, step: int, conversation_id: int):
        ...
        for tool_call in tool_calls:
            ...
            if function_to_call:
                console.print(f'Calling tool: {tool_call.function.name}, with arguments: {tool_call.function.arguments}', end='')
                
                try:
                    result = function_to_call(**tool_call.function.arguments)
                    console.print(f'[bold magenta]Tool result:' +'{result}')
    ...
    def _append_assistant_message_with_thinking(self, messages: list, content: str, thinking: str) -> list:
        """
